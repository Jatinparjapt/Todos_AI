...
const All = () => {
  ...
  const getComp = useSelector((state)=>state.setDataLocal.comp) // get component name
  console.log(getComp, "getcomp")

  const validStatuses = ["Progress", "Completed", "Pending"];
  const filterData = validStatuses.includes(getComp)
    ? todoList.filter((item) => item.status === getComp)
    : todoList;
    console.log(filterData, "fielte")
  ...
  useEffect(() => {
    ...
    // dispatch(getTodoList(1035780732));
            ...
            {filterData.map((task, index) => (
              <tr key={index} className="rounded-sm border-2 my-4">
                <td
                  ...
                  className="md:mx-1 cursor-pointer"
                >
                  {task.title}
                </td>
                <td
                ...
                <td
                  ...
                  className={`md:mx-1 flex cursor-pointer items-center font-semibold ${
